...
```
这个`null`一直在给我产生bug。尽管这个*this*很烦人，但是因为JS有内置的关于偏函数应用的方法，所以还稍微方便了一些。然而，大多数FP程序员更倾向于使用他们选择的FP库里提供的`partial(..)`方法。  

### 参数翻转
回忆下我们的Ajax函数的签名：`ajax( url, data, cb )`。如果我们想对`cb`做偏函数应用但是又想等之后再指定`data`和`url`呢？我们可以新建一个方法来包括这个函数来翻转它的参数顺序：
```js

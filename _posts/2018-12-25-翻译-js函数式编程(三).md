...
```
现在想象下我们有一串数字，然后对其中的每一个都加上一个特定的数。我们会使用JS数组内置的方法`map(..)`(具体看[第九章，”Map“](https://github.com/getify/Functional-Light-JS/blob/master/manuscript/ch9.md/#map))  
```js
...
// [4,5,6,7,8]
```
我们不能直接把`add(..)`传入`map(..)`的原因是`add(..)`的签名与`map(..)`期望的mapper函数不匹配。这就是偏函数应用起作用的地方：我们可以调整`add(..)`的函数签名直到匹配某些东西：
```js
...
### `bind(..)`
JS的函数都有一个内置的方法叫做`bind(..)`。它有两个功能：预设置`this`上下文及偏函数参数。  

...
```
`normalizeArrayChildren`方法接收两个参数，一个是children表示子节点，nestedIndex表示嵌套的数组的索引。这个方法主要的逻辑就是遍历children，获得单个节点c，如果c是数组类型就递归调用`normalizeArrayChildren`并更新nestedIndex，如果是文本节点就创建TextVNode，如果是像v-for这样嵌套的数组children，就根据nestedIndex更新key。在这三种情况中，都做了一种处理，就是如果有连续的text节点，把它们合并在一起。  
经过这样的处理，返回的就是一个规范的VNode了。  
回到`create-element.js`，接下来就是创建VNode实例了
```js
let vnode, ns
if (typeof tag === 'string') {
  let Ctor
...
```
这里首先判断tag类型，如果是string就接着判断是否是内置标签，是的话就创建一个普通VNode。然后通过`resolveAsset`方法判断是否是已注册的组件，是就通过`createComponent`方法创建一个VNode，不是就创建一个未知标签的VNode。如果tag类型不是string就直接通过`createComponent`创建一个VNode。这个方法我们先跳过，后期单独来讲。继续回到`create-element.js`里
```js
 if (Array.isArray(vnode)) {
    return vnode
  } else if (isDef(vnode)) {
...
```
这里的逻辑很简单，就是判断vnode是否按规范生成了，如果没有就返回一个空的vnode。这里还有一个`registerDeepBindings`方法
```js
// ref #5318
...
```
根据注释及代码来看这应该是用来解决当:style和:class是对象时的响应问题的。
关于render部分基本讲完了，下一章继续回到init.js中。
...
是的；`=>`箭头函数是词法匿名的；没有办法在语法上为它设置一个名字。它们的名字可以向常规的函数一样推断出来，但是再一次，最常见的通过arguments来传递函数值的情况是不被支持的。真是让人讨厌。  
如果出于某些原因`person.nicknames`没有定义，那就会抛出一个异常，意味着这个`(anonymous function)`就会出现在堆栈跟踪的最上层。  
老实说，`=>`箭头函数的匿名性就像一把匕首插在我的心上。我不能忍受没有函数名。这样使得我们更难去阅读，更难去debug，以及无法做到自我引用。  
...
```
在FP世界里关于`=>`的让人激动的例子主要是由于它几乎完全数学上对函数的定义，特别是像Haskell这样的FP语言。箭头函数的原型就是通过数学方式来沟通的。  
再深入一点，我支持`=>`的论点是，通过使用更轻量的语法，我们模糊了函数之间的视觉边界，这让我们使用简单的函数表达式就像我们使用惰式的表达式一样--另一个FP开发者的最爱。  
我认为大多数FP开发者都不在乎我分享的这些问题。他们喜欢匿名函数，他们喜欢省略语法。但是就像我之前说的：你自己决定。  
**注意**：虽然我不推荐在生产环境使用`=>`，但是它们在快速的代码探索中是很有用的。另外，在这本书剩下的部分我们会有很多地方用到箭头函数--特别是当我们提到典型的FP技术时--提倡简洁，将代码占用的物理空间优化到极限。你自己决定这种方式让你的预生产环境代码是具有更多函数更少的可读性。  

### this是什么？
如果你还不熟悉JS里的`this`的绑定规则，我推荐你看下我的书*你不知道的JS：this&对象原型*。出于这一章的目的，我假设你已经了解了函数调用时`this`是如何定义的(四条规则之一)。但是如果你还对于`this`模糊不清，那好消息是我们将会得出一个结论，那就是如果你要用FP那就吧应该用`this`。  
**注意：**我们正在讨论一个我们最终决定不用的话题。为什么？因为关于`this`的话题与本书之后降到的其它话题有潜在的联系。举个例子，我们关于函数纯粹度的概念被`this`影响，这实际上是函数的隐形输入(具体看[第五章](https://github.com/getify/Functional-Light-JS/blob/master/manuscript/ch5.md))。另外，我们对于`this`的观点影响到我们选择数组方法(`arr.map(..)`)还是单独的函数(`map(..,arr)`)(具体看[第九章](https://github.com/getify/Functional-Light-JS/blob/master/manuscript/ch9.md))。理解`this`对于理解为什么`this`确实不应该用在你的FP代码中是必须的！  
JS的`function`有一个`this`关键字，这是每一个函数在调用时都会绑定的。`this`关键字可以有很多种解释方式，但是我认为它为函数运行提供了一个对象上下文。  
...
sum( context );
```
这样更简单了。并且这种类型的代码在FP里也更容易处理。这样很容易把多个函数连在一起，或者使用任何一种我们将在下一章讲的引发争议的技术，当输入始终是显式的时候。
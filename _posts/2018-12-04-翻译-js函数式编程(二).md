...
除了在debug问题上为函数命名还有其它好处。首先，语法名(又称词法名)，对内部的自我引用有帮助。自我引用对于递归(包括同步和异步)是必须的，并且对于事件处理也有帮助。  
考虑以下不同的场景：
```js
// sync recursion:
...
// ..
```
函数命`getPreferredName(..)`告诉读者了一部分这个map操作准备做的事，而这光看代码不能完全确定。这个名称标记让代码更具可读性。  
...
```
事实上你从来没有看到IIFEs的函数表达式有函数名，但是它们应该有。为什么？为了我们刚刚说过的理由：堆栈追踪调试，可靠的自我引用以及可读性。如果你实在想不出名字，至少可以用IIFE：
```js
(function IIFE(){
...
### 没有`function`的函数
目前为止我们都是用的标准的函数语法。但是你毫无疑问也听过了网络上关于ES6`=>`箭头函数的议论。  
对比一下：
```js
people.map( function getPreferredName(person){
    return person.nicknames[0] || person.firstName;

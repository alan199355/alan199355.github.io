...
```
**注意：**`Object.assign(..)`是一个ES6+的特性，用来从一个或多个源对象浅复制到一个单独的目标对象：`Object.assign( target, source1, ... )`。  
...
这段代码出于各种原因与FP的许多原则都不相符，但是其中最明显的一个问题是隐式的`this`分享。我们可以更具显式风格并且保持代码更具FP友好风格：
```js
// ..
...
// ..
```
从我的角度来说，问题不在于通过对象来组织行为。而在于我们试图用隐式的输入而不是显式的.当我戴上了FP的帽子，我就想把`this`束之高阁。  

### 总结
函数是很强大的。  
但是我们要清楚函数究竟是什么。它不仅仅是声明和操作的集合。具体来说，一个函数需要一个或多个输入(理想情况是只要一个)和一个输出。  
函数内部的函数可以有对外部变量的闭包并且之后都可以记住它们。这是在所有编程领域都是最重要的概念之一，并且是FP的基础知识。  
小心匿名函数，尤其是箭头函数。它们写起来很方便，但是它们把成本从作者转移到读者了。我们学习FP的最大理由就是写出更具可读性的代码，所以不要这么快就跟着潮流。  
不要在函数内用this。不要用就对了。  
现在你的脑海了应该有一个清晰的观点关于函数在FP里意味着什么。现在是时候用函数来做一些交互工作了，下一章会教你各种接下来要用到的关键技术。